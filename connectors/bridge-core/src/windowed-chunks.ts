...
export type WindowedCaptureChunk = Readonly<{
  document: WindowedCaptureBatchDocument;
  bytes: Buffer;
}>;
...
function hasWindowCoordinates(
  ...
  return (
    ...
    value.window_discriminator === windowDiscriminator
  );
}

function oversizeEvent(
  sessionID: string,
  windowDiscriminator: string,
): CanonicalJson {
  return {
...
export function normalizeWindowedCaptureEvent(
  value: unknown,
  sessionID: string,
  windowDiscriminator: string,
): CanonicalJson {
...
export function buildWindowedCaptureChunks(input: {
  bootstrap: BootstrapBinding;
  batchID: string;
  sessionID: string;
  windowDiscriminator: string;
  ...
  try {
    ...
    const events = input.events.map((event) => {
      ...
      return encodedSize({
        ...
        ? normalized
        : oversizeEvent(input.sessionID, input.windowDiscriminator);
    });
...
export function inspectWindowedChunkCoverage(
  chunks: readonly WindowedCaptureBatchDocument[],
): CaptureChunkCoverage {
  try {
    if (!Array.isArray(chunks) || chunks.length === 0) throw new BridgeContractError();
    const first = chunks[0]!;
    if (
      !SHA256.test(first.window_discriminator) ||
